...
Per-stop swap shipped (`app/api/itinerary/swap.ts`, `POST /api/itinerary/[id]/swap`): user taps an UPCOMING stop, types a complaint, the engine acts on intent — VENUE (replace, hold slot), TIME (move the slot), or CONSTRAINT (re-search). Distinct from reroute (external disruption → downstream replan). `floorTime()` shared from `store.ts` by both engines.
- **Time swaps**: `parseTimeExpr` handles relative ("an hour earlier" = -60, vague = ±30) + absolute ("after 8" = 20:00); it's the deterministic floor under the Groq interpret. `resettleTail` is the reusable **try → adapt → notify** ladder (shift + reflow, keep venue if open at the new time, else re-search an equivalent, else fail loud). Plan-then-commit: a failed resettle leaves the itinerary untouched. Duration/extend swaps will reuse `resettleTail` — keep it generic.
