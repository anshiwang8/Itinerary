...
The 4 manual-testing bugs (fail-loud surface, gibberish message, constraint enforcement, price refresh) and 2 UI items (swap input spaces, venue description) are SHIPPED, live-verified, AND locked in as e2e scenario tests — `e2e/failloud.spec.ts` (bad-input messages pinned exact-text) + `e2e/scenarios.spec.ts` (price refresh, description, spaces, repeated swaps, swap-then-reroute, active-stop rejection), 16/16 @mock, deterministic across runs. Nothing actively in flight; GTFS remains deferred. When adding scenario tests, reuse `planExpectingProblem` / `swapOn` / `expectStripMatchesPin` from `e2e/helpers.ts` and the fixture triggers in `e2e/README.md`.
